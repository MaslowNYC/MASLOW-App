...
// Sample options for Quick Visit (2 max) and Reservation (5 max)
const SAMPLE_OPTIONS = [
  { id: 'face_wash', label: 'Face Wash Towelette' },
  { id: 'spf', label: 'SPF Face Lotion' },
  { id: 'mouthwash', label: 'Mouthwash' },
  { id: 'deodorant', label: 'Deodorant Wipe' },
  { id: 'linen_towel', label: 'Fresh Linen Towel' },
  { id: 'hand_lotion', label: 'Hand Lotion' },
...
// Sample checkbox with disabled state for limit enforcement
...
}> = ({ label, value, selected, disabled, onToggle }) => {
  // Can always deselect, but can only select if not disabled
  const isDisabled = disabled && !selected;

  return (
    <TouchableOpacity
      style={[
        styles.sampleCheckboxRow,
        isDisabled && styles.sampleCheckboxDisabled,
      ]}
    ...
    >
      <View style={[
        styles.sampleCheckbox,
        selected && styles.sampleCheckboxSelected,
      ]}>
        {selected && <Ionicons name="checkmark" size={16} color={colors.cream} />}
      </View>
      <Text style={[
        styles.sampleCheckboxLabel,
        isDisabled && styles.sampleCheckboxLabelDisabled,
      ]}>
        {label}
      </Text>
    </TouchableOpacity>
...
export default function EditProfileScreen() {
  ...
  // Accordion state - all sections collapsed by default
  const [expandedSections, setExpandedSections] = useState({
    personalInfo: false,
    suitePreferences: false,
    amenities: false,
    quickVisitSamples: false,
    usage: false,
  });
  ...
  const loadProfile = async () => {
    try {
      ...
      const profileData: ProfileData = {
        ...
        default_preferences: {
          ...defaultSuitePreferences,
          ...data?.default_preferences,
          quick_visit_samples: data?.default_preferences?.quick_visit_samples || [],
          reservation_samples: data?.default_preferences?.reservation_samples || [],
        },
  ...
  const toggleArrayPreference = (
    field: 'preferences_amenities' | 'preferences_usage',
    id: string
  ) => {
    haptics.light();
    setProfile(prev => {
  ...
  // Toggle quick visit sample (max 2)
  const toggleQuickVisitSample = (value: string) => {
    setProfile(prev => {
      const current = prev.default_preferences.quick_visit_samples || [];
      if (current.includes(value)) {
        ...
        return {
          ...
          default_preferences: {
            ...
            quick_visit_samples: current.filter(s => s !== value),
          },
        };
      } else {
        // Add if under limit
        if (current.length < 2) {
          ...
          return {
            ...
            default_preferences: {
              ...prev.default_preferences,
              quick_visit_samples: [...current, value],
            },
          };
        } else {
          haptics.error();
          Alert.alert('Limit Reached', 'Quick visits include 2 samples. Book a reservation for more!');
          return prev;
        }
      }
    });
      ...
      >
        ...
        >
          ...
          <View style={styles.section}>
            <TouchableOpacity
              style={styles.sectionHeader}
              onPress={() => toggleSection('quickVisitSamples')}
              activeOpacity={0.7}
            >
              <View style={styles.sectionHeaderLeft}>
                <View style={styles.sectionHeaderRow}>
                  <Text style={styles.sectionTitle}>QUICK VISIT SAMPLES</Text>
                  <Text style={styles.sampleCount}>
                    {(profile.default_preferences.quick_visit_samples || []).length}/2
                  </Text>
                </View>
                <Text style={styles.sectionSubtitle}>Default samples for 10-minute quick visits</Text>
              </View>
              <Ionicons
            ...
            <View style={styles.sectionContent}>
              {SAMPLE_OPTIONS.map((option) => {
                const samples = profile.default_preferences.quick_visit_samples || [];
                const isSelected = samples.includes(option.id);
                const isAtLimit = samples.length >= 2;
                return (
                  <SampleCheckbox
                    key={option.id}
                    label={option.label}
                    ...
                    selected={isSelected}
                    disabled={isAtLimit}
                    onToggle={toggleQuickVisitSample}
                  />
                );
              })}
            </View>
