...
export default function BookingFlowScreen() {
  ...
  useEffect(() => {
    ...
    if (currentSamples.length > maxSamples) {
      setBookingData(prev => ({
        ...
        preferences: {
          ...prev.preferences,
          samples: currentSamples.slice(0, maxSamples)
        }
      }));
  ...
  const loadBookingData = async () => {
    try {
      ...
      // Load available passes (stored in credits table)
      ...
      setAvailablePasses(totalPasses);
      ...
      if (profileData?.default_preferences) {
        const { quick_visit_samples, ...otherPrefs } = profileData.default_preferences;
        setBookingData(prev => ({
          ...prev,
          preferences: {
            ...prev.preferences,
            ...otherPrefs,
            // Pre-populate with Quick Visit samples (user can change these freely)
            samples: quick_visit_samples || [],
          },
        }));
      }
      ...
      >
          ...
          <View style={styles.singleScreenContainer}>
            ...
            <View style={styles.ultraCompactSection}>
                  ...
                  <View style={styles.paymentToggle}>
                    <TouchableOpacity
                      style={[
                        ...
                        paymentMethod === 'passes' && styles.paymentToggleOptionActive,
                      ]}
                      onPress={() => {
                        haptics.light();
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (!canAfford) {
                          ...
                          Alert.alert(
                            'Insufficient Passes',
                            `You need ${option.passes} pass${option.passes > 1 ? 'es' : ''} for ${option.minutes} minutes. You have ${availablePasses}.`,
                            [
                              { text: 'Pay Cash', onPress: () => setPaymentMethod('cash') },
                              { text: 'Buy Passes', onPress: () => router.push('/buy-credits') },
                    ...
                    >
                      ...
                      <Text style={[
                        ...
                        {paymentMethod === 'passes' ? `${option.passes} pass${option.passes > 1 ? 'es' : ''}` : `$${option.cash}`}
                      </Text>
              ...
              {availablePasses === 0 && (
                ...
                >
                  ...
                  <Text style={styles.buyCreditsLinkText}>Buy Passes & Save</Text>
                </TouchableOpacity>
              )}
              {availablePasses > 0 && availablePasses < 8 && (
                <Text style={styles.creditBalanceHint}>
                  You have {availablePasses} pass{availablePasses !== 1 ? 'es' : ''} • Need more?{' '}
                  <Text
            ...
            <View style={styles.ultraCompactSection}>
              ...
              <View style={styles.timeWindowUltraGrid}>
                {[
                  { id: 'morning', label: 'AM', time: '9-12', icon: 'sunny-outline' },
                  { id: 'afternoon', label: 'PM', time: '12-5', icon: 'partly-sunny-outline' },
                  { id: 'evening', label: 'Eve', time: '5-8', icon: 'moon-outline' },
        ...
        {step === 'environment' && (
          <View style={styles.envContainer}>
            ...
            {/* Bidet & Heated Seat Row */}
            <View style={styles.envRowCards}>
              {/* Bidet Card */}
              <View style={styles.envCardHalf}>
                <Text style={styles.envCardTitle}>Bidet</Text>
                <View style={styles.envChipRow}>
                  {[
                    ...
                    <TouchableOpacity
                      ...
                      style={[
                        styles.envChipSmall,
                        bookingData.preferences.bidet_temp === option.value && styles.envChipSmallSelected,
                      ]}
                      onPress={() => {
                        haptics.light();
                        setBookingData(prev => ({
                    ...
                    >
                      <Text style={[
                        styles.envChipSmallText,
                        bookingData.preferences.bidet_temp === option.value && styles.envChipSmallTextSelected,
                      ]}>{option.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={styles.envCardTitle}>Heated Seat</Text>
                <View style={[
                  styles.toggleSwitchEnv,
                  bookingData.preferences.heated_seat && styles.toggleSwitchEnvActive,
                ]}>
                  <View style={[
                    styles.toggleKnobEnv,
                    bookingData.preferences.heated_seat && styles.toggleKnobEnvActive,
                  ]} />
                </View>
              </TouchableOpacity>
            </View>

            {/* Ambience Card - Button Grid with Lit Indicators */}
            <View style={styles.envCard}>
              <Text style={styles.envCardTitle}>Ambience</Text>
              <View style={styles.ambienceGrid}>
                {[
                  { value: 'silence', label: 'Silence', icon: 'volume-mute' },
                  { value: 'your-music', label: 'Your Music', icon: 'musical-notes' },
                  { value: 'white-noise', label: 'White Noise', icon: 'water' },
                  { value: 'flushing', label: 'Otohime', icon: 'leaf' },
                  { value: 'spa', label: 'Spa', icon: 'flower' },
                  { value: 'classical', label: 'Classical', icon: 'disc' },
                ].map((option) => {
                  const isSelected = bookingData.preferences.music === option.value;
                  return (
                    <TouchableOpacity
                      key={option.value}
                      style={[
                        styles.ambienceButton,
                        isSelected && styles.ambienceButtonSelected,
                      ]}
                      onPress={() => {
                        haptics.light();
                        setBookingData(prev => ({
                          ...prev,
                          preferences: { ...prev.preferences, music: option.value as BookingData['preferences']['music'] },
                        }));
                      }}
                      activeOpacity={0.7}
                    >
                      <View style={[
                        styles.ambienceIndicator,
                        isSelected && styles.ambienceIndicatorLit,
                      ]} />
                      <Ionicons
                      ...
                      <Text style={[
                        ...
                        isSelected && styles.ambienceLabelSelected,
                      ]}>{option.label}</Text>
                    </TouchableOpacity>
                  );
                })}
                  ...
                  {[
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          if (isSelected) {
                            // Always allow deselection (even if it was a Quick Visit default)
                            haptics.light();
                            setBookingData(prev => ({
                              ...prev,
                              preferences: {
                            ...
                            setBookingData(prev => {
                              ...
                              if (prev.preferences.samples.length < currentMax) {
                                ...
                                haptics.error();
                                Alert.alert('Limit Reached', `You can select up to ${currentMax} samples for this duration.`);
            ...
            <MaslowCard padding="md">
              ...
              <View style={styles.creditSummary}>
                <View style={styles.creditSummaryRow}>
                  ...
                  <Text style={styles.creditSummaryValue}>1 Pass</Text>
                </View>
              </View>
              <Text style={styles.creditBalance}>
                Balance after booking: {availablePasses - 1} pass{availablePasses - 1 !== 1 ? 'es' : ''}
              </Text>
            </MaslowCard>
          </View>
        )}
      ...
      {step !== 'success' && (
        <View style={styles.footer}>
          <View style={styles.creditBadge}>
            ...
            <Text style={styles.creditText}>{availablePasses} pass{availablePasses !== 1 ? 'es' : ''}</Text>
          </View>
          <MaslowButton
...
const styles = StyleSheet.create({
  ...
  // Ambience Button Grid Styles - Compact
  ambienceGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 5,
  ...
  // Legacy Environment Styles (keep for other uses)
  musicKnobCompact: {
    alignItems: 'center',
    paddingVertical: spacing.xs,
  ...
  // Continuous Slider Styles - Compact
